...
import { TrendingUp, Award, MapPin, Briefcase, ArrowRight, Calculator as CalculatorIcon } from 'lucide-react';
...
const ProfessionPage = ({ 
  ...
  const t = {
    ...
    calculateCTA: language === 'fr' ? 'Calculer mon tarif personnalisé' : 'Calculate my personalized rate',
    avgRatesTitle: language === 'fr' ? 'Tarif moyen en 2026' : 'Average rate in 2026',
    ...
    experienceTitle: language === 'fr' ? 'Tarif selon l\'expérience' : 'Rate by experience',
    locationTitle: language === 'fr' ? 'Tarif selon la localisation' : 'Rate by location',
    skillsTitle: language === 'fr' ? 'Compétences qui paient le plus' : 'Top-paying skills',
    skillBonus: language === 'fr' 
      ? (bonus) => `Augmente le TJM de ${bonus}€ en moyenne`
      : (bonus) => `Increases daily rate by €${bonus} on average`,
    negotiationTitle: language === 'fr' ? '💰 Comment négocier votre tarif' : '💰 How to negotiate your rate',
    faqTitle: language === 'fr' ? '❓ Questions fréquentes' : '❓ Frequently asked questions',
    relatedTitle: language === 'fr' ? 'Voir aussi' : 'See also',
    avgRate: language === 'fr' ? 'TJM moyen' : 'Average daily rate',
    finalCTATitle: language === 'fr' ? 'Calculez votre tarif personnalisé' : 'Calculate your personalized rate',
    finalCTASubtitle: language === 'fr'
    ...
    finalCTAButton: language === 'fr' ? 'Calculer maintenant (Gratuit)' : 'Calculate now (Free)',
    perDay: language === 'fr' ? '/jour' : '/day',
    perMonth: language === 'fr' ? '/mois' : '/month',
  };

  return (
    <div className={`min-h-screen ${darkMode ? "dark" : ""} bg-gradient-to-br from-purple-50 via-pink-50 to-blue-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900`}>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 mb-8">
          ...
          <div className="grid md:grid-cols-3 gap-6 mb-8">
            <div className="p-6 bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-xl border-2 border-purple-200 dark:border-purple-700">
              <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">{t.hourlyRate}</p>
            ...
            <div className="p-6 bg-gradient-to-br from-blue-50 to-cyan-50 dark:from-blue-900/20 dark:to-cyan-900/20 rounded-xl border-2 border-blue-200 dark:border-blue-700">
              <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">{t.dailyRate}</p>
              <p className="text-3xl font-bold text-blue-600 dark:text-blue-400">
                {data.avgRates.daily}€<span className="text-lg">{t.perDay}</span>
              </p>
            </div>
            <div className="p-6 bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 rounded-xl border-2 border-green-200 dark:border-green-700">
              <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">{t.monthlyRate}</p>
              <p className="text-3xl font-bold text-green-600 dark:text-green-400">
                {data.avgRates.monthly}€<span className="text-lg">{t.perMonth}</span>
              </p>
            </div>
          </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 mb-8">
          ...
          <div className="space-y-4">
            {data.experienceBreakdown.map((level, idx) => (
              <div key={idx} className="flex items-center justify-between p-5 bg-gray-50 dark:bg-gray-900 rounded-xl hover:shadow-md transition-shadow">
                <div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 mb-8">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {data.locationBreakdown.map((loc, idx) => (
              <div key={idx} className="flex items-center justify-between p-5 bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 rounded-xl">
                <div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 mb-8">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {data.topSkills.map((skill, idx) => (
              <div key={idx} className="p-5 bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 rounded-xl border border-green-200 dark:border-green-800">
        ...
        <div className="bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-2xl shadow-xl p-8 mb-8 border-2 border-purple-200 dark:border-purple-700">
          ...
          <div className="space-y-4">
            {data.negotiationTips.map((tip, idx) => (
              <div key={idx} className="flex gap-4 p-4 bg-white dark:bg-gray-800 rounded-xl">
