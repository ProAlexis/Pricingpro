...
const ProfessionComparison = ({ currentFormData, onCompare, language = 'fr' }) => {
  ...
  const calculateRateForProfession = (professionValue) => {
    ...
    const experienceMultiplier = 1 + (parseInt(currentFormData.experience) * 0.08);
    ...
    const locationMultiplier = locationMultipliers[currentFormData.location] || 1;
  ...
  const addComparison = () => {
    ...
    if (comparisons.some(c => c.professionValue === selectedProfession)) {
      alert(language === 'fr' ? 'Cette profession est déjà dans la comparaison' : 'This profession is already in the comparison');
      return;
    }
  ...
  const removeComparison = (professionValue) => {
    setComparisons(comparisons.filter(c => c.professionValue !== professionValue));
  };

  // Calculer le tarif actuel
  const currentProfession = professions.find(p => p.value === currentFormData.profession);
  const currentRate = onCompare ? calculateRateForProfession(currentFormData.profession) : null;
            ...
            <div className="flex-1 overflow-y-auto p-6">
              ...
              {comparisons.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500 dark:text-gray-400">{t.noComparison}</p>
                </div>
              ) : (
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  {currentRate && (
                    <div className="bg-gradient-to-br from-purple-600 to-pink-600 text-white rounded-xl p-6">
                      <div className="flex items-center justify-between mb-4">
                        <span className="text-sm opacity-90">{t.currentProfession}</span>
                        <TrendingUp className="w-5 h-5" />
                      </div>
                      <h3 className="text-xl font-bold mb-4">{currentRate.profession}</h3>
                      <div className="space-y-3">
                        <div>
                          <p className="text-sm opacity-75">{language === 'fr' ? 'Tarif journalier' : 'Daily rate'}</p>
                          <p className="text-3xl font-bold">{formatCurrency(currentRate.daily)}</p>
                        </div>
                        <div className="pt-3 border-t border-white/20">
                          <p className="text-sm opacity-75">{language === 'fr' ? 'Tarif mensuel' : 'Monthly rate'}</p>
                          <p className="text-xl font-semibold">{formatCurrency(currentRate.monthly)}</p>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {comparisons.map((comparison) => {
                    const diff = currentRate ? getDifference(comparison.daily, currentRate.daily) : null;
                    ...
                    return (
                      ...
                      >
                        <button
                          onClick={() => removeComparison(comparison.professionValue)}
                        ...
                        <div className="space-y-3">
                          ...
                          {diff && !diff.isSame && (
                            <div className={`flex items-center gap-2 text-sm ${diff.isHigher ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                              <ArrowRight className={`w-4 h-4 ${diff.isHigher ? 'rotate-45' : 'rotate-[-45deg]'}`} />
                              <span className="font-semibold">
                                {diff.isHigher ? `+${diff.percentage}%` : `-${diff.percentage}%`}
                              </span>
                          ...
                          <div className="pt-3 border-t border-gray-200 dark:border-gray-600">
                            <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">
                              {language === 'fr' ? 'Tarif mensuel' : 'Monthly rate'}
                            </p>
