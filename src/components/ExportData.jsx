...
const ExportData = ({ results, formData, language = 'fr' }) => {
  ...
  const translations = {
    fr: {
      ...
      exported: "Exporté !",
      description: "Téléchargez vos données pour les utiliser dans Excel, Google Sheets ou d'autres outils."
    },
    en: {
      ...
      exported: "Exported!",
      description: "Download your data to use in Excel, Google Sheets or other tools."
    }
  };
  ...
  const exportCurrentCalculation = () => {
    const headers = language === 'fr' 
      ? ['Profession', 'Localisation', 'Expérience (années)', 'Niveau', 'Compétences', 'Tarif Horaire', 'Tarif Journalier', 'Tarif Mensuel', 'Date']
      : ['Profession', 'Location', 'Experience (years)', 'Level', 'Skills', 'Hourly Rate', 'Daily Rate', 'Monthly Rate', 'Date'];
    ...
    const csvContent = [
      headers.join(','),
      objectToCSVRow(data, headers)
    ].join('\n');

    downloadCSV(csvContent, `PricingPro_Calcul_${new Date().toISOString().split('T')[0]}.csv`);
  };

  // Exporter tout l'historique en CSV
  const exportHistory = () => {
    try {
      const history = JSON.parse(localStorage.getItem('pricingpro_history') || '[]');
      ...
      const headers = language === 'fr'
        ? ['Date', 'Profession', 'Localisation', 'Expérience', 'Niveau', 'Compétences', 'Tarif Horaire', 'Tarif Journalier', 'Tarif Mensuel']
        : ['Date', 'Profession', 'Location', 'Experience', 'Level', 'Skills', 'Hourly Rate', 'Daily Rate', 'Monthly Rate'];
      ...
      downloadCSV(csvContent, `PricingPro_Historique_${new Date().toISOString().split('T')[0]}.csv`);
    } catch (error) {
  ...
  const exportToExcel = () => {
    ...
    if (history.length === 0) {
      ...
      downloadExcel(tableHTML, `PricingPro_Calcul_${new Date().toISOString().split('T')[0]}.xls`);
    } else {
      ...
      downloadExcel(tableHTML, `PricingPro_Historique_${new Date().toISOString().split('T')[0]}.xls`);
    }
  };
