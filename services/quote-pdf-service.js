...
export function generateQuotePDF(quoteData, language = 'fr') {
  ...
  const freelanceNameWithEI = quoteData.legalStatus === 'auto-entrepreneur' 
  ...
  const addressToDisplay = quoteData.legalStatus === 'portage' 
  ...
  const siretToDisplay = quoteData.legalStatus === 'portage' 
  ...
  doc.text(`${quoteData.daysCount} ${labels.days}`, pageWidth - 90, yPos, { align: 'center' });
  doc.text(`${quoteData.dailyRate}€`, pageWidth - 60, yPos, { align: 'center' });
  ...
  const finalTotalLabel = quoteData.legalStatus === 'auto-entrepreneur' ? labels.totalNet : labels.totalTTC;
  ...
  if (quoteData.legalStatus === 'auto-entrepreneur') {
    doc.setFontSize(8);
    doc.setTextColor(100, 100, 100);
    doc.text("TVA non applicable, art. 293 B du CGI", pageWidth / 2, pageHeight - 40, { align: 'center' });
  }
