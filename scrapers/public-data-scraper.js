...
export async function scrapePublicDataRates() {
  console.log('🔍 Scraping PUBLIC DATA rates (Malt, Free-Work, Stack Overflow)...');
  console.log('📊 Source: Baromètre Malt 2024-2025, Free-Work IT, Stack Overflow 2024\n');
  
  const rates = [];
  const experienceLevels = ['junior', 'mid', 'senior'];
  let geocodingCount = 0;
  
  for (const [professionKey, experienceData] of Object.entries(MALT_REAL_DATA_2025)) {
  ...
  console.log(`📊 Coverage: ${Object.keys(MALT_REAL_DATA_2025).length} professions × ${Object.values(CITIES_BY_COUNTRY_REAL).flat().length} cities × 3 experience levels`);
  console.log(`📜 Sources: Baromètre Malt 2024-2025, Free-Work IT Earnings, Stack Overflow Survey 2024\n`);
